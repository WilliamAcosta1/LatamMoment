...
module.exports = {
    async getConfig(guild) {
        ...
        // Select latinoland database
        var db = mongoClient.db("latinoland");
    ...
    getZumitoSettings() {
        var settings = JSON.parse(localStorage.getItem('latinoland.settings'));
        return settings || {};
    },

    setZumitoSettings(json) {
        localStorage.setItem('latinoland.settings', JSON.stringify(json))
    },
}